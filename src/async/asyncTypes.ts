...
async function raceExamples(): Promise<void> {
  const result = await Promise.race([
    fetchData(),
    calculate(),
    new Promise<string>((resolve) => setTimeout(() => resolve("Fastest"), 100))
  ]);
...
interface AsyncIterableExample {
  [Symbol.asyncIterator](): AsyncIterator<number>;
}

class Counter implements AsyncIterableExample {
...
// 使用异步迭代器
async function iterateAsync(): Promise<void> {
...
// 13. 条件类型与异步编程
// Awaited<T> 是 TypeScript 4.5+ 提供的条件类型，用于提取 Promise 解析后的类型
type UnwrappedPromise<T> = Awaited<T>;

type StringPromise = Promise<string>;
type UnwrappedString = UnwrappedPromise<StringPromise>; // string

type NestedPromise = Promise<Promise<number>>;
type UnwrappedNumber = UnwrappedPromise<NestedPromise>; // number
